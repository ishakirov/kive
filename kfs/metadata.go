...
func (md *Metadata) fetchManifest(streamName, sourceName, hour string) ([]ktypes.ChunkInfo, error) {
	...
	value = md.infoCache.Get(ktypes.ComposeHourKeyCache(streamName, sourceName, hour))
	if value != nil {
		md.infoCache.Set(ktypes.ComposeHourKeyCache(streamName, sourceName, hour), &result, md.config.RemovalTime.GetDuration()-2*time.Hour)
	}
	return result, nil
}
...
func (md *Metadata) writeFsInfo(key innerStorage, fullname string) error {
	...
	md.m.Lock()
	defer md.m.Unlock()
	md.infoCache.Set(ktypes.ComposeHourKeyCache(key.StreamName, string(key.StreamType), key.HourString()), &updatedChunkInfoList, md.config.RemovalTime.GetDuration()-2*time.Hour)
	return nil
}
...
func (md *Metadata) writeFsInfoFallback(key innerStorage, fullname string) error {
	...
	md.infoCache.Set(fullname, &key, md.config.RemovalTime.GetDuration()-2*time.Hour)
	return nil
}
...
func (md *Metadata) readFsInfo(key innerStorage, fullname string) (*innerStorage, error) {
	...
	md.infoCache.Set(fullname, &key, time.Duration(key.Ts)-md.config.RemovalTime.GetDuration()-2*time.Hour)
	return &jData, nil
}
